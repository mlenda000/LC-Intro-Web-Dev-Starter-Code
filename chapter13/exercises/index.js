...
const average = require('./ScoreCalcs/averages.js');
  ...
  if (response.toLowerCase()==='y'){
    if (i===0){
      ...
      for (let j = 0; j<testTitles.length; j++){
        let avg = average.averageForTest(j,scores)
        console.log(`${testTitles[j]} test average = ${avg}%.`);
      }
    } else if (i===2){
      for (let j = 0; j<astronauts.length; j++){
        let avg = average.averageForStudent(j,scores) //Call 'averageForStudent' here. Pass in j and scores as arguments.
